...
<div id="accordion">
            ...
            if(isset($genotype_selection_arr) && is_array($genotype_selection_arr) && !empty($genotype_selection_arr)) {
                ...
                for ($i = 0; $i < count($genotype_selection_arr); $i++) {
                    if ($genotype_selection_arr[$i]->Genotype == $genotype) {
                        echo "<input type=\"checkbox\" id=\"genotype_" . $genotype_selection_arr[$i]->Genotype . "\" name=\"genotype_" . $genotype_selection_arr[$i]->Genotype . "\" value=\"" . $genotype_selection_arr[$i]->Genotype . "\" checked><label for=\"genotype_" . $genotype_selection_arr[$i]->Genotype . "\" style=\"margin-right:10px;\">" . $genotype_selection_arr[$i]->Genotype . "</label>";
                    } else {
    ...
    if(isset($phenotype_selection_arr) && is_array($phenotype_selection_arr) && !empty($phenotype_selection_arr)) {
        ...
        echo "<button onclick=\"queryPhenotypeDescription('" . $organism . "')\" style=\"margin-right:20px; background-color:#FFFFFF;\">Download Phenotype Description</button>";
    }
    @endphp
    <button onclick="queryVariantAndPhenotype('{{$organism}}')" style="margin-right:20px; background-color:#99DDFF;">View Data</button>
    <button onclick="downloadVariantAndPhenotype('{{$organism}}')" style="margin-right:20px; background-color:#FFFFFF;">Download Data</button>
    </div>
    <br/><br/>
