...
<body>
    ...
    if (isset($cnv_accession_count_result_arr) && is_array($cnv_accession_count_result_arr) && !empty($cnv_accession_count_result_arr) && !is_null($cnv_accession_count_result_arr)) {
        ...
        for ($i = 0; $i < count($cnv_accession_count_result_arr); $i++) {
            ...
            echo "<td>";
            echo "<a href=\"" . route('system.tools.GenVarX.viewCNVAndImprovementStatus', ['organism'=>$organism, 'Chromosome'=>$cnv_accession_count_result_arr[$i]->Chromosome, 'Position_Start'=>$cnv_accession_count_result_arr[$i]->Start, 'Position_End'=>$cnv_accession_count_result_arr[$i]->End, 'CNV_Data_Option'=>$cnv_data_option]) . "\" target=\"_blank\">";
            echo "<button>View Details</button>";
            echo "</a>";
            echo "</td>";
            echo "<td>";
            echo "<a href=\"" . route('system.tools.GenVarX.viewCNVAndPhenotype', ['organism'=>$organism, 'Chromosome'=>$cnv_accession_count_result_arr[$i]->Chromosome, 'Position_Start'=>$cnv_accession_count_result_arr[$i]->Start, 'Position_End'=>$cnv_accession_count_result_arr[$i]->End, 'CNV_Data_Option'=>$cnv_data_option]) . "\" target=\"_blank\">";
