                ...
                if (!empty($motif_result_arr) && count($motif_result_arr) > 0) {
                    ...
                    for ($j = 0; $j < count($motif_result_arr); $j++) {
                        ...
                        foreach ($motif_result_arr[$j] as $key => $value) {
                            if ($key == 'Binding_TF') {
                                echo "<td style=\"border:1px solid black; min-width:80px;\"><a target=\"_blank\" href=\"" .
                                    route('system.tools.GenVarX.viewPromoterOnSelectedBindingTF', [
                                        'organism' => $organism,
                                        'Motif' => $value,
                                        'Gene' => $motif_result_arr[$j]->Gene,
                                        'Chromosome' => $motif_result_arr[$j]->Binding_Chromosome,
                                        'Motif_Start' => $motif_result_arr[$j]->Binding_Start,
                                        'Motif_End' => $motif_result_arr[$j]->Binding_End,
                                        'Gene_Binding_Sequence' => $motif_result_arr[$j]->Gene_Binding_Sequence,
