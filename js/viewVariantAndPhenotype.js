...
function constructInfoTable(organism, res, chromosome, position, genotype_array) {
    ...
    for (let i = 0; i < header_array.length; i++) {
        ...
        if (header_array[i] == "Chromosome" || header_array[i] == "Position" || header_array[i] == "Accession" || header_array[i] == "GRIN_Accession" || header_array[i] == "Genotype" || header_array[i] == "Category" || header_array[i] == "Imputation" || header_array[i] == "Improvement_Status" || header_array[i] == "Admixture_Group" || header_array[i] == "Subpopulation") {
            ...
            detail_a.href = "../viewVariantAndPhenotypeFigures/"+organism+"?chromosome_1=" + chromosome + "&position_1=" + position + "&phenotype_1=" + header_array[i] + "&genotype_1=" + genotype_array.join("%0D%0A");
            ...
            success: function (response) {
                ...
                if (res.length > 0) {
                    document.getElementById('Variant_and_Phenotye_detail_table').appendChild(
                        constructInfoTable(organism, res, chromosome_1, position_1, genotype_array)
                    );
...
function downloadVariantAndPhenotype(organism) {
